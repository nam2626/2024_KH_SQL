...
--부서별, 직급별, 인원수, 연봉 평균(소수점 절삭)
SELECT EMP_DEPARTMENT, EMP_POSITION, 
		COUNT(*) AS EMP_COUNT, TRUNC(AVG(EMP_SALARY),0) AS AVG_SALARY 
FROM EMPLOYEE
GROUP BY EMP_DEPARTMENT, EMP_POSITION; 

--부서별, 인원수, 연봉 평균(소수점 절삭)
...
--전체 인원수, 연봉 평균(소수점 절삭)
SELECT COUNT(*) AS EMP_COUNT, TRUNC(AVG(EMP_SALARY),0) AS AVG_SALARY 
FROM EMPLOYEE; 
...
SELECT CAR_MAKER, CAR_MAKE_YEAR, 
		COUNT(*) AS CAR_COUNT, TRUNC(AVG(CAR_PRICE),0) AS AVG_PRICE  
FROM CAR
