...
SELECT 
	TO_CHAR(CS.CAR_SELL_DATE,'YYYY') AS SELL_YEAR, CM.CAR_MAKER_NAME, 
	SUM(CS.CAR_SELL_EA) AS SUM_EA, SUM(CS.CAR_SELL_PRICE) AS SUM_PRICE,
	TRUNC(AVG(CS.CAR_SELL_PRICE),0) AS AVG_PRICE
FROM CAR C INNER JOIN CAR_SELL CS 
ON C.CAR_ID = CS.CAR_ID
INNER JOIN CAR_MAKER CM
ON C.CAR_MAKER_CODE = CM.CAR_MAKER_CODE
