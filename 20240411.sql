...
IS
	USER_EXCEPTION EXCEPTION;
BEGIN
	DBMS_OUTPUT.PUT_LINE(PID || ' ' || NAME || ' ' || AGE);	
	INSERT INTO PERSON VALUES(PID,NAME,AGE);
	IF AGE < 0 THEN
		RAISE USER_EXCEPTION;
	END IF;
	COMMIT;
	DBMS_OUTPUT.PUT_LINE('데이터 등록 완료');
EXCEPTION
	WHEN DUP_VAL_ON_INDEX THEN
		DBMS_OUTPUT.PUT_LINE('데이터가 중복 되었습니다.');
	WHEN USER_EXCEPTION THEN
		DBMS_OUTPUT.PUT_LINE('나이는 0보다 커야 됩니다.');
		ROLLBACK;--UPDATE,DELETE,INSERT 한 작업을 취소
END;

/

BEGIN
	PROCEDURE_EX5('0014','김철수1',-9);
